...
from Utils.io import save_model, insert_into_table, read_img
...
if __name__ == "__main__":
    for organism in ORGANISMS:
        ...
        for concentration in concentrations:
            ...
            labels = [name_to_abbr.get(x, x) for x in os.listdir(f"{new_root}/train")]
            ...
            param_grid = {
                'pca__n_components': range(10, 57, 1),
                'lda__solver': ['svd', 'eigen']
            }
            grid = GridSearchCV(pipeline, param_grid=param_grid, cv=10, n_jobs=-1, verbose=1)
            grid.fit(X_train, y_train)
            best_model = grid.best_estimator_
            save_model(best_model, f"{MODEL}/{IDENTIFIER}_{organism}_{concentration}.pkl")
